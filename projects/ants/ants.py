...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            ), 'Too many ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current_place:
            ...
            if bee and self.lower_bound <= distance <= self.upper_bound:
                return bee  
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        for elem in self.place.bees[:]:
            elem.reduce_health(self.damage)
# END Problem 9
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        find_behind = self.place.exit
        
        while find_behind:
            if find_behind.ant:
                if find_behind.ant.is_container and (find_behind.ant.ant_contained is not None and isinstance(find_behind.ant.ant_contained, Ant)) and (not find_behind.ant.ant_contained.is_doubled):
                    find_behind.ant.ant_contained.damage *= 2
                    find_behind.ant.ant_contained.is_doubled = True
                elif not find_behind.ant.is_doubled:
                    find_behind.ant.damage *= 2
                    find_behind.ant.is_doubled = True
            find_behind = find_behind.exit
        # END Problem 12
